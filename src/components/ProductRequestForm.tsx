...
const requestSchema = z.object({
  ...
  message: z.string().trim().min(5, "Please describe what you need").max(500),
});
    ...
    return (
      <Card className="border-dashed border-2 border-primary/30 bg-primary/5 hover:border-primary/50 transition-all cursor-pointer" onClick={() => setIsOpen(true)}>
        <CardContent className="p-6 text-center">
          ...
          <h3 className="font-outfit font-semibold text-lg mb-1">Can't find what you need?</h3>
          <p className="text-sm text-muted-foreground">
            Click here to request any product not listed
          </p>
        </CardContent>
      </Card>
    );
